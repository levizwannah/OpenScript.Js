...
class Compiler extends OpenScript.AutoLoader {
    /**
     * 
     * @param  {string} fileName  the name of the file without the .js extension 
     * 
     * A file is created with all the classes from the file and the classes separated with
     * commas
     */
    async classesFromFile(fileName) {
        try {
            const filePath = path.join(this.dir, this.normalize(fileName) + this.extension);
            ...
            const finalCode = `function ojs(${wrappedCode})`;

            
            const outputFile = path.join(this.dir, 'output.js');
            fs.writeFileSync(outputFile, finalCode, 'utf-8');

            console.log(`Wrapped code has been written to ${outputFile}`);
